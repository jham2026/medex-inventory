...
export default function AdminAccounts() {
  ...
  const [saving, setSaving]     = useState(null); // accountId being saved
  ...
  async function assignRep(accountId, repId) {
    ...
    if (error) {
      ...
      setAccounts(prev => prev.map(a =>
        a.id === accountId
          ? { ...a, assigned_rep_id: repId || null }
          : a
      ));
      toast.success('Rep assigned!');

      // Mark related unassigned_account alerts as read
      if (repId) {
    ...
    <div>
      ...
      {unassignedCount > 0 && (
        <div className="alert-banner warning" style={{ marginBottom: 14 }}>
          ⚠ {unassignedCount} accounts have no rep assigned. Use the filter below to find them quickly.
